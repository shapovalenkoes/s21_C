...
call_head = """Suite *s21_math_suite(void) {
  ...
  TCase *tc = tcase_create("case_math");"""
...
def make_cases():
    ...
    open_bracket = "{"
    close_bracket = "}"
    values = [0.0, -0.0, -1.2, 1.2, -21.0, 21.0, -0.01, 0.01, 21.9, -21.9, "1.79769313486231570e+308", "2.2250738585072014E-308"]
    for value_ in values:    
        case = "START_TEST(test_{0}) {1}\nlong double my_val = s21_floor({2});\nlong double test_val = floor({2});\nck_assert_ldouble_eq(my_val, test_val);\n{3}\nEND_TEST\n".format(number_test, open_bracket, value_, close_bracket)
        cases.append(case)
        number_test += 1
    values = [0.0, -0.0, -1.2, 1.2, -21.0, 21.0, -0.01, 0.01, 21.9, -21.9, "1.79769313486231570e+308", "2.2250738585072014E-308"]
    for value_ in values:    
        case = "START_TEST(test_{0}) {1}\nlong double my_val = s21_ceil({2});\nlong double test_val = ceil({2});\nck_assert_ldouble_eq(my_val, test_val);\n{3}\nEND_TEST\n".format(number_test, open_bracket, value_, close_bracket)
        cases.append(case)
        number_test += 1
    return cases
